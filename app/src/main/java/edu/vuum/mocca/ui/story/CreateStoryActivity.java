...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchCameraIntent() {
		...
		//Intent imgIntent = new Intent(this, MediaStore.ACTION_IMAGE_CAPTURE.getClass());
		...
		//Uri imgUri = getOutputMediaFileUri(MEDIA_TYPE_IMAGE);
		 ...
		 Uri fileImageUri = getOutputMediaFileUri(MEDIA_TYPE_IMAGE) ;
		 fragment.imagePath = fileImageUri;
		
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		
		imgIntent.putExtra(MediaStore.EXTRA_OUTPUT, fragment.imagePath);
		//imgIntent.putExtra(MediaStore.EXTRA_OUTPUT,imgUri.getPath());
		
		
		// TODO - Start a new activity for result, using the new intent and the request
		// code CAMERA_PIC_REQUEST
		if (imgIntent.resolveActivity(getPackageManager()) != null) {
			startActivityForResult(imgIntent, CAMERA_PIC_REQUEST);
		}
		//this.startActivityForResult(imgIntent, CAMERA_PIC_REQUEST);
	...
	private void launchVideoCameraIntent() {
		...
		// getOutputMediaFile to create a new filename for this specific video;
		 ...
		 Uri fileImageUri = getOutputMediaFileUri(MEDIA_TYPE_VIDEO) ;
		 fragment.imagePath = fileImageUri;
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
